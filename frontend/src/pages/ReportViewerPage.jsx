    ...
    <div className="space-y-5 pb-12">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-4 shadow-card flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="font-extrabold text-slate-900 text-sm bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 focus:outline-none focus:border-brand-500 cursor-pointer pr-4"
              >
                {reports.map((r) => (
        ...
        <div className="flex items-center gap-2 shrink-0">
          {/* Mobile view toggle */}
          <div className="md:hidden flex bg-slate-100 p-0.5 rounded-lg border border-slate-200 text-xs font-bold">
            <button
              onClick={() => setMobileView('source')}
              className={`px-2.5 py-1 rounded ${mobileView === 'source' ? 'bg-white text-brand-700 shadow-xs' : 'text-slate-600'}`}
            >
              Source Text
            </button>
            <button
              onClick={() => setMobileView('structured')}
              className={`px-2.5 py-1 rounded ${mobileView === 'structured' ? 'bg-white text-brand-700 shadow-xs' : 'text-slate-600'}`}
            >
              Structured Data
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className={`lg:col-span-5 bg-white rounded-2xl border border-slate-200 p-5 shadow-card flex flex-col h-[740px] ${mobileView === 'structured' ? 'hidden md:flex' : 'flex'}`}>
                ...
                {currentReport.pages_metadata.map((p) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded font-bold font-mono transition ${
                      selectedPage === p.page_num ? 'bg-sky-50 text-brand-700 border border-sky-200 shadow-xs' : 'text-slate-600 hover:text-slate-900'
                    }`}
                  >
                    Pg {p.page_num}
                  </button>
          ...
          <div className="mt-3 flex-1 overflow-y-auto bg-slate-900 text-slate-200 p-3.5 rounded-xl font-mono text-xs leading-relaxed space-y-0.5 select-text shadow-inner">
          ...
          <div className="pt-2.5 border-t border-slate-100 mt-2 text-[10.5px] text-slate-500 flex items-center justify-between shrink-0 font-mono">
                  ...
                  <div
                    ...
                    className={`p-3.5 rounded-xl border transition ${
                      obs.is_reviewed
                        ...
                        : 'bg-white border-slate-200 hover:border-brand-300 shadow-subtle'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          {obs.is_reviewed && (
                            <span className="text-[10px] uppercase font-bold px-2 py-0.5 bg-emerald-50 text-emerald-800 border border-emerald-200 rounded-md flex items-center gap-1 font-mono">
                              <CheckCircle className="w-3 h-3 text-emerald-600" /> {obs.review_status}
                            </span>
                          )}
                        </div>
