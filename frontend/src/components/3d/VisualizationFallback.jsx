    ...
    <div className="bg-slate-900 text-white rounded-2xl p-6 border border-slate-800 space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-widest text-sky-400">2D Relationship Matrix</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-slate-800/80 rounded-xl border border-slate-700 space-y-2">
          ...
          <div className="text-sm font-extrabold text-white">{patient?.name || 'No Patient'}</div>
        ...
        <div className="p-4 bg-slate-800/80 rounded-xl border border-slate-700 space-y-2">
          ...
          {reports.length === 0 ? (
            ...
            <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
              {reports.map((r) => (
                <div key={r.id} className="text-xs bg-slate-900/80 p-2 rounded-lg border border-slate-700/60 flex items-center justify-between">
                  <span className="font-semibold text-white truncate max-w-[170px]">{r.title}</span>
                  <span className="text-[10px] font-mono text-slate-400">{r.report_date || '2026'}</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="p-4 bg-slate-800/80 rounded-xl border border-slate-700 space-y-2">
          ...
          {conflicts.length === 0 ? (
            <div className="text-xs text-emerald-400 flex items-center gap-1.5 pt-2">
              <CheckCircle2 className="w-4 h-4" /> No active clinical conflicts detected
            </div>
          ) : (
            <div className="space-y-1.5 max-h-36 overflow-y-auto pr-1">
              {conflicts.map((c) => (
                <div key={c.id} className="text-xs bg-amber-500/10 border border-amber-500/30 p-2 rounded-lg text-amber-200">
      ...
      <div className="space-y-3 pt-2">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {observations.map((obs) => (
            <div key={obs.id} className="p-3.5 bg-slate-800/90 rounded-xl border border-slate-700/80 space-y-2">
              <div className="flex items-start justify-between gap-2">
                <span className="font-bold text-white text-xs">{obs.test_name}</span>
              ...
              <div className="text-[11px] text-sky-300 font-mono bg-slate-900/80 p-1.5 rounded border border-slate-800">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-700/60">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="px-2 py-0.5 bg-brand-600 hover:bg-brand-500 text-white font-bold rounded text-[10.5px] flex items-center gap-1 transition"
                  >
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 bg-slate-700 hover:bg-slate-600 text-slate-200 font-semibold rounded text-[10.5px] transition"
                  >
                    Review
                  </button>
                </div>
