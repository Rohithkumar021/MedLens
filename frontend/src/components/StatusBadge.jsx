...
export default function StatusBadge({ status, size = 'md', showReasonTooltip = false, reason = '' }) {
  ...
  const sizeClasses = {
    sm: 'text-[10.5px] px-2 py-0.5 gap-1 font-semibold tracking-wide',
  ...
  return (
    <span
      ...
      className={`inline-flex items-center rounded-md border shadow-subtle select-none font-mono ${config.bgClass} ${sizeClasses[size]}`}
    >
